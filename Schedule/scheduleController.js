...
exports.getSchedule = (req, res) => {
  ...
  const isZaOchnayaGroup = true; // Замените это значение на фактический способ определения заочной группы

  const queryZaOchnayaGroup = `
    SELECT
    ...
    WHERE dek_zgroup_predmet.id_group = ${id_group}
      ...
      ${isZaOchnayaGroup ? "AND WEEKOFYEAR(dek_zgroup_predmet.date) = WEEKOFYEAR(NOW())" : ""}
    ORDER BY dek_zgroup_predmet.date ASC
  `;

  const queryRegularGroup = `
    SELECT
    ...
    WHERE dek_group_predmet.id_group = ${id_group}
      ...
      ${isZaOchnayaGroup ? "AND dek_group_predmet.chetnechet IS NULL" : ""}
      AND dek_group_predmet.day >= DAYOFWEEK(CURDATE())
    ORDER BY FIELD(weekday, 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'), numberPair ASC
  `;

  db.query(isZaOchnayaGroup ? queryZaOchnayaGroup : queryRegularGroup, (error, rows) => {
    const timeIntervals = [
      "8:30-10:00",
      "10:10-11:40",
      "12:10-13:40",
